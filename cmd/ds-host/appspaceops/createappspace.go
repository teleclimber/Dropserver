...
func (c *CreateAppspace) Create(dropID domain.DropID, appVersion domain.AppVersion, baseDomain, subDomain string) (domain.AppspaceID, error) {
	...
	if !check.Valid || !check.Available { // validitiy should be checked at route handler?
		return domain.AppspaceID(0), errors.New("domain invalid or unavailable") // this is unhelpful.
	}
	...
	_, err = c.MigrationJobModel.Create(dropID.UserID, appspace.AppspaceID, appVersion.Version, true)
	if err != nil {
		return domain.AppspaceID(0), err
	}

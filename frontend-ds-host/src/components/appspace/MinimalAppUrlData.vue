...
<template>
	<div class="flex">
		...
		<svg v-if="url_data.last_result === 'error' || url_data.new_url" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
			<path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
		</svg>
		<span v-if="url_data.last_result === 'error'">There was a problem fetching the app listing</span>
		<span v-else-if="url_data.new_url">App has moved to a new address</span>

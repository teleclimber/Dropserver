...
const invitesStore = useAdminUserInvitesStore();
invitesStore.fetch();
...
const invalid = computed( () => {
	...
	if( invitesStore.is_loaded && invitesStore.invites.find( i => i.email === e ) ) {
		return 'Email already in list of invitations';
	}
	return ''; 	
});
...
async function saveClicked() {
	...
	saving.value = true;
	save_rejected.value = await invitesStore.createInvitation(email.value);
	saving.value = false;
...
<div class="rounded border border-yellow-200 p-3 bg-yellow-100">
	<form @submit.prevent="saveClicked" @keyup.esc="$emit('close')">
		<label>Email of person to invite:</label>
		<input 
			type="text"
			ref="email_input"
			name="email"

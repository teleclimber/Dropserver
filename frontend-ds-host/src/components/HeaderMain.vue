...
const display_image_url = computed(() => authUserStore.getDisplayImageUrl());
...
<template>
	<header class="fixed w-full md:w-auto md:relative border-b bg-white grid ds-header-phone md:ds-header-full">
		...
		<div class="justify-self-end self-center pr-4 md:pr-6 flex-initial ">
			<div class="w-8 h-8 md:w-12 md:h-12 rounded-full bg-blue-100 border-2 border-blue-300 text-blue-400 flex justify-center items-end cursor-pointer hover:bg-blue-200 overflow-hidden" @click="openUserMenu">
				<img v-if="display_image_url" :src="display_image_url" class="w-full h-full object-cover" />
				<svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 md:w-10 md:h-10">

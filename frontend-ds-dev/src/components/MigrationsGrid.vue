...
const grid = computed( () => {
	...
	const ret :{schema:number, up:string, down:string}[] = [];
	for( let i=from_schema; i<=to_schema; ++i ) {
		let down = !!m.find( g=> g.schema === i && g.direction === "down") ? "present" : "missing";
		if( down === "missing" && i === 1 ) down = "missing-ok";
		ret.push({
			schema: i,
			up: !!m.find( g=> g.schema === i && g.direction === "up") ? "present" : "missing",
			down
		});
	}
	return ret;
...
<template>
	<table v-if="grid.length" class="border">
		<thead>
			<tr>
				<th class="px-1">schema:</th>
				<th v-for="g in grid">{{ g.schema }}</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				...
				<td v-for="g in grid" class="px-1">
					<svg v-if="g.up === 'present'" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-big-up-filled text-green-500" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
			...
			<tr>
				...
				<td v-for="g in grid" class="px-1">
					<svg v-if="g.down === 'present'" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-big-down-filled text-green-500" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
					...
					<svg v-else-if="g.down === 'missing' || g.down === 'missing-ok'" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x " :class="[ g.down === 'missing-ok' ? 'text-gray-300': 'text-orange-500']" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
						<path d="M18 6l-12 12"></path>
						<path d="M6 6l12 12"></path>
					</svg>
